...
# Change the date
add_step! (RubyApp::Element::ExecutedEvent)                   { |event| event.assert_exists_text(RubyApp::Language.locale.strftime(event.today, Pike::Application.configuration.format.date.short)) }
...
# A midnight transition stops a started task and changes the date
...
add_step! (RubyApp::Element::UpdatedEvent)                    { |event| event.assert('Pike::Session.identity.user.work.where_started.count == 1') { Pike::Session.identity.user.work.where_started.count == 1 } }
add_step! (RubyApp::Element::AssertedEvent)                   { |event| event.now = event.now + 24 * 60 * 60
                                                                        event.trigger_element(Pike::Session.document.page) }
add_step! (RubyApp::Element::TriggeredEvent)                  { |event| event.assert('Pike::Session.identity.user.work.where_started.count == 0') { Pike::Session.identity.user.work.where_started.count == 0 } }
add_step! (RubyApp::Element::AssertedEvent)                   { |event| event.assert_exists_text(RubyApp::Language.locale.strftime(event.today, Pike::Application.configuration.format.date.short)) }
