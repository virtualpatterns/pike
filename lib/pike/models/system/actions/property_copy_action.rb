            ...
            unless self.user_target
              ...
              self.user_source.friendships_as_source.all.each do |friendship|
                unless self.property
                  ...
                  #self.sync_all_properties_to_friend(friendship.user_target)
                else
                  # Sync a specific property to a friend
                  self.sync_property_to_friend(self.property, friendship.user_target)
                end
              end
            else
              ...
              if self.user_source.friendships_as_source.where_user_target(self.user_target).exists?
                ...
                unless self.property
                  ...
                  self.sync_all_properties_to_friend(self.user_target)
                else
                ...
                self.sync_all_properties_to_non_friend(self.user_target)
              end
            end
          end
        end

        def sync_all_properties_to_friend(user)
          self.user_source.properties.all.each do |property|
            self.sync_property_to_friend(property, user)
          end
        end

        def sync_property_to_friend(property, user)
          # Add or update the friend's property
          self.add_update_property_to_user(property, user)
        end

        def sync_all_properties_to_non_friend(user)
          self.user_source.properties.all.each do |property|
